  ...
  >
    <svg aria-hidden="true" viewBox="0 0 20 20" class="icon like-icon">
      <g fill="none" fill-rule="evenodd">
        ...
        <path
          :stroke="likedToShow ? '#37C700' : '#8A93A0'"
          stroke-linejoin="round"
          :fill="likedToShow ? '#37c700' : 'none'"
...
const likedToShow = computed(() => {
  return store.isLogin && liked.value;
})
const likesCount = ref(props.likesCount);
const likedStatus = ref(props.liked);
...
const handleLikeClick = () => {
  ...
  timer = setTimeout(() => {
    timer = null;
    if (liked.value !== likedStatus.value) {
      emit("like-click", liked.value);
      likedStatus.value = liked.value;
    }
  }, 1500);
};
